Read every .md file in docs/schoolzen-planning/v1/leave/ in full — this task builds ONLY the "Requests" page, but the whole module's .md files give the shared context (schema relationships, cross-page rules) you need.
...
Match docs/schoolzen-planning/v1/leave/requests.html pixel-for-pixel.

Specific rules for this page: both Staff and Student can request leave with different filter shapes (Dept+Designation vs Class+Section); the four distinct actions (Approve/Reject/Cancel/Delete) are never merged.
...
New collections/fields and new API routes only — do not modify any existing legacy schema or route in place. Wire this page behind /v2/leave/requests; do not touch or redirect the legacy equivalent yet.
