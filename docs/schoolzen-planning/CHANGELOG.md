...
## v1 (new) — 2026-09-06
- NEW: prompts/ — 14 ready-made Claude Code prompt files, numbered in
  build order (00-shared-components through 13-dashboard), each a
  complete single-line-usable prompt ("claude \"$(cat
  prompts/NN-module.md)\"") that tells Claude Code exactly which .md/
  .html files to read and which specific rules to follow for that
  module, restates the isolation rules (new schema/routes only,
  feature-flagged behind /v2/, stop before cutover for manual
  verification) inline so they don't get missed, and calls out each
  module's real dependencies (e.g. Approvals needs Leave already
  merged, Settings needs Student already stable, Dashboard needs
  nearly everything). prompts/README.md explains usage and the
  strict build order.

## v1 (restructure) — 2026-09-06
- Restructured prompts/ from one-prompt-per-module to ONE PROMPT PER
  PAGE, grouped into a folder per module (e.g. prompts/07-payroll/
  01-generate-payroll.md through 04-assign-salary.md) - a whole module
  in one Claude Code task was too large a change to safely review;
  now each page is its own small, independently-verifiable task.
  Every page prompt states that page's own specific rules pulled from
  its .md file, notes real cross-module dependencies inline, and
  stops after feature-flagging for manual verification before
  cutover. 34 page prompts total across 12 module folders, plus
  standalone 00-shared-components.md (built first) and
  13-dashboard/01-dashboard.md (built last). prompts/README.md
  rewritten to explain the new structure and usage.
- NEW: `_core/frontend-backend-folder-structure.md` - the actual
  codebase organization convention for both Angular frontend and
  Express backend: module-first folders (app/payroll/,
  server/modules/payroll/), with one page-subfolder per page inside
  each module on the frontend (matching each .html reference's
  filename) and one routes file per page on the backend - never a
  flat components/routes/models folder mixing all 35+ pages together.
  Explains why this matters at scale (adding module 14 next year only
  means adding one new sibling folder, never reorganizing what
  already exists) and calls out Approvals as the one exception (no
  models folder, since it's a computed view over other modules' data).

## v1 (fix) — 2026-09-06
- Backend folder structure revised: added a controllers/ folder inside
  every module (one file per page, holding the actual handler logic),
  keeping routes/ files thin (just HTTP verb+path wiring to a
  controller function) - a standard, more maintainable Express split
  than routes holding logic directly. Added a module-root
  <module>.utils.js convention for logic shared ACROSS a module's own
  pages only (e.g. Payroll's gross-salary calculation used by both
  Generate Payroll and Salary Payouts) - explicitly distinct from the
  global shared/ folder, which is reserved for what most/all modules
  need (error handling, logging), not a dumping ground for anything
  reused twice. Updated all 36 page prompts to reference building
  "the controller function(s) and thin route file" instead of just
  "the API route(s)", so Claude Code follows this split from the
  first page it builds.

## v1 (fix) — 2026-09-06
- Backend folder structure revised to match the ACTUAL existing
  project layout (backend/modules/{controllers,models,routes,helpers,
  middleware,services,queues,workers,validators,sockets} as sibling
  technical-layer folders — confirmed from a screenshot of the real
  schoolzen-advanced/backend project) instead of the module-first
  structure proposed earlier. Top-level layout is preserved exactly;
  what's new is a module-named subfolder INSIDE each existing layer
  folder (controllers/payroll/, models/payroll/, routes/payroll/,
  helpers/payroll/ when needed) so a module's files stay grouped and
  identifiable across layers without touching the project's existing
  top-level structure. Documented where services/, queues/, workers/,
  validators/, and sockets/ fit into this same pattern too. Updated
  00-shared-components.md to clarify the backend structure already
  exists and must not be recreated or renamed - only the frontend's
  shared/ component library is built fresh.

## v1 (new) — 2026-09-06
- NEW: `_core/shell/app-shell.html` + `app-shell.md` — the DEFINITIVE
  header+sidebar component, replacing the static header/sidebar
  snapshot that was copy-pasted into every one of the 35 page
  references throughout this package. Built from the legacy
  side-navbar/header-navbar (Admin) and side-nav/header-nav (Teacher)
  components, preserving and extending their real behavior:
  - Sidebar regrouped from a flat ~20-item unscannable list into 13
    collapsible module sections (accordion, one open at a time,
    current page's group auto-expanded).
  - Role-aware exactly matching the legacy's two real patterns:
    admin-only groups/items (Staff, Academic Setup, Holiday, Settings,
    and several individual pages) disappear entirely for Teacher, vs.
    permission-gated items (Admission, Student, Fees, Attendance,
    Marksheet, Admit Card, Transfer Certificate, Class Promote) that
    stay VISIBLE but show a lock icon and no click handler when that
    specific R3 permission is false - reproducing the legacy's
    `*ngIf="xPermission"` / lock-icon-else pattern precisely.
  - Genuinely responsive: a real CSS media query at 860px collapses
    the sidebar into an off-canvas drawer with a backdrop and
    hamburger toggle - not a simulated mobile view, resizing the
    actual browser demonstrates it.
  - Added a notification bell (unread dot, dropdown, "View all" link)
    per the unified Notifications service proposed in additional-
    technical-considerations.md - new, not in the legacy header.
  - Dropped the legacy Admin header's "UPGRADE PLAN" button (SaaS
    billing upsell, unrelated to this package's product surface).
  - The Admin/Teacher toggle and lock-icon behavior in the reference
    file is a demo control only - the real app reads these states
    from the logged-in staff member's actual roles/permissions.
- Updated prompts/00-shared-components.md to build this shell FIRST,
  before the shared component library - every other page renders
  inside it.

## v1 (FINAL — sidebar/header sync) — 2026-09-06
- Fixed a package-wide gap: all 38 individual page reference files
  (every module page + Dashboard) were still carrying the OLD flat,
  ungrouped sidebar snapshot from before the App Shell redesign -
  batch-updated every single one to the new 13-group accordion sidebar
  (`_core/shell/app-shell.html`'s structure), each page correctly
  showing its own module's group open, its own page highlighted as
  active-sub, and every other group collapsed - verified across
  Academic Setup, Payroll, and Examination as spot-checks, script
  reported 38/38 updated with zero pattern-match failures.
- Added the notification bell to every page's header (was previously
  only shown in the app-shell.html demo file) - same icon+badge
  markup, consistent across all 38 pages now.
- Package marked FINAL. This closes out the App Shell rollout: the
  shell reference, its design doc, and every page's own header/
  sidebar snapshot are now fully consistent with each other.
